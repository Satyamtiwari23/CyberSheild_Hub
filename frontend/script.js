...
function updateUI() {
    ...
    if (userEmail && userName) {
        ...
        if (accountBtn) accountBtn.textContent = firstLetter;
        if (authBtn) authBtn.textContent = "Logout";

    } else {
        if (userNameSpan) userNameSpan.textContent = "Guest";
        if (userEmailSpan) userEmailSpan.textContent = "guest@cybershield.local";
...
function updateThreatLevel(value) {
    ...
    if (!threatFill || !threatBadge || !threatDesc || !sysStatus) {
        return;
    }
    ...
    const dot =
        sysStatus.querySelector('.status-dot');

    const labelEl =
        sysStatus.querySelector('.status-label');
...
function randomThreatUpdate() {
    ...
    const current =
        parseFloat(threatFill.style.width) || 12;
    ...
    if (Math.random() < 0.08) {
        ...
        newVal =
            Math.min(
                92,
                newVal + 20 + Math.random() * 20
            );

        addActivity(
            '⚠️',
            'Spike in suspicious network traffic detected!'
        );
    }
    ...
    if (Math.random() < 0.05) {
        ...
        newVal =
            Math.max(2, newVal - 15);

        addActivity(
            '✅',
            'Threat level decreasing — systems stabilizing.'
        );
    }
...
scanBtn?.addEventListener('click', () => {
    if (
        ...
        console.warn("Some scan elements are missing from the page.");
        return;
    }
